...
def basePath = gitblit.getRepositoriesFolder().getAbsolutePath().replaceAll("/", Matcher.quoteReplacement("\\"))
def repoName = repository.name.replaceAll("/", Matcher.quoteReplacement("\\"))
// define the trigger url
def triggerUrl = jenkinsUrl + "/git/notifyCommit?url=$basePath\\$repoName"
logger.info("jenkins hook triggering url: ${triggerUrl}")
// trigger the build
